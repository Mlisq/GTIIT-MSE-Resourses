...
#Part a
r_xNumbers = np.sort(np.random.uniform(-5,5,11))
...
#Part d
ch_Tangents = itp.CubicSpline(r_xNumbers, r_yNumbers)
...
randomX = int(np.random.uniform(0,256,1))   #Little complex in order to highlight the noise point
...
f_xValues = np.arange(len(r_syValues))
